...
test('review generation source keeps progress feedback in a dismissible floating dock', () => {
  ...
  assert.match(reviewGenerationSource, /getReviewTaskDockLessons\(lessons, visibleFailedVersionIds\)/);
  ...
  assert.match(workspacePageContentSource, /apiFetch<unknown>\('\/api\/review-plans\/failure-notifications'\)/);
  assert.match(workspacePageContentSource, /\/api\/review-plans\/failure-notifications\/seen/);
  assert.match(workspacePageContentSource, /offset \+= 100/);
  assert.match(workspacePageContentSource, /versionIds\.slice\(offset, offset \+ 100\)/);
  assert.match(workspacePageContentSource, /setReviewFailureNoticesByVersionId[\s\S]*REVIEW_FAILED_TASK_AUTO_DISMISS_MS/);
  assert.match(workspacePageContentSource, /reviewFailureReceiptRequestedIds/);
  ...
  assert.match(workspacePageContentSource, /const reviewDockLessons = getReviewTaskDockLessons\(reviewTaskDockSourceLessons, visibleFailedReviewVersionIds\);/);
  ...
  assert.match(workspacePageContentSource, /<ReviewGenerationTaskDock[\s\S]*lessons=\{reviewTaskDockSourceLessons\}[\s\S]*notice=\{reviewFloatingNotice\}/);
