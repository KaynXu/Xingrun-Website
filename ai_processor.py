...
- reason_blank_prompt: string，用于第一个书写区。请写成多行字符串：第一行是这个书写区的小标题；后续内容必须是挖空题正文，不要写成开放问答。每个书写区至少包含 3 个 ______ 空格，并尽量把留白放在整句的关键位置，方便孩子直接填写
- improvement_summary_prompt: string，用于第二个书写区。请写成多行字符串：第一行是这个书写区的小标题；后续内容也必须是挖空题正文，不要写成大段自由总结。重点让孩子补出“缺的是哪个知识点、接下来准备怎么补这个知识点、以后怎么提醒自己”
...
6. reason_blank_prompt 聚焦“这题为什么错”，要让孩子补出具体错因、漏掉的条件、看错的符号、顺序或关系；如果涉及知识点不牢，要明确点出孩子缺的是哪个知识点、规则或判断依据。
7. improvement_summary_prompt 聚焦“接下来怎么把缺的知识点补上、以后如何避免再错”，仍然要写成挖空题，不要变成解题教学。
8. 每个书写区至少包含 3 个 ______ 空格，且空格尽量分布在整句关键位置，避免只挖很短的一两个词。
9. 不要在挖空题后面再追加纯写字线、自由总结、长段说明或“请写一写”的作文式提示。
10. 如果是细节问题，优先围绕检查顺序、符号、单位、小数点、抄写和验算来组织提示。
11. 如果是审题问题，优先围绕圈条件、看清关键词、对应图形或已知信息来组织提示。
12. 如果是方法问题，优先围绕先判断方法是否合适、为什么会选错方法来组织提示。
13. 如果是知识点问题，优先围绕先回忆规则、定义或常用判断依据，再写清接下来怎么补这个知识点。
