...
- 当前实现与工作流文档不一致：文档要求 `原题 / 原图 -> 方法提醒 -> 挖空复盘 -> 订正区`，但最新吴靖萱 PDF 文本统计里这四个标题全是 `0`，只出现 `题目内容` 和 `重做这题`。
- 当前 UI 更像“把内容放进卡片和横线”，还不是稳定的错题本版式；方法提醒、挖空复盘、订正区没有被明确视觉分区，老师反馈区也未出现。
...
- 不建议现在直接训练模型；当前更应该先做 eval、标注字段、规则库候选和 PDF 回归样例集。
...
### 问题：PDF 模板没有兑现工作流要求的四段结构

- 证据：
  - `/Users/xiaodi/Desktop/xingrun.web/docs/wrong-questions/wrong-question-review-workflow.md:67` 要求每题固定顺序为 `原题 / 原图`、`方法提醒`、`挖空复盘`、`订正区`。
...
- 修改建议：先改 PDF schema/template：把两个 prompt 拆成 `method_hint_lines` 与 `blank_review_blocks`，模板显式渲染四段标题；保留现有 prompt 作为内容来源。
- 是否需要人工确认：需要。需确认四段标题是否必须显式出现，以及“重做这题”是否等同订正区。
...
- 视觉层级：当前封面清楚，但题页层级不足。`第 N 题`、`题目内容`、写作卡、`重做这题` 是弱语义；建议题页改为四个显式区块：`原题 / 原图` 最大、`方法提醒` 小而高亮、`挖空复盘` 核心、`订正区` 留白。
...
判断：当前卡片能让学生重做一部分题，但不像完整错题本。它更偏“读提示 + 重做”，没有稳定地引导学生经历 `入手判断 -> 关键条件 -> 方法选择 -> 推理/计算 -> 结论/易错点`。
...
1. **P0：把 PDF 渲染模板改回四段显式结构。**
   - 为什么：当前最新 PDF 不显示工作流要求的四区。
   - 验收标准：PDF 文本中每题都有 `原题 / 原图`、`方法提醒`、`挖空复盘`、`订正区`，顺序正确。
...
5. **P1：把内容 schema 从两个 prompt 拆成教学结构。**
   - 为什么：现在方法提醒和挖空复盘混在写作卡里。
   - 验收标准：中间 JSON 至少包含 `method_hint_lines[]`、`blank_review_blocks[]`、`teacher_feedback`、`confirmation_reasons[]`。
...
本文应作为错题本系统后续迭代的评估基线。后续如要实施，建议另开任务分支，并按优先级先处理 P0：四段结构、老师确认机制、几何题质量审稿。实施时不要直接改总控 prompt，应先补 schema、模板、质量检查和回归样例，再决定是否需要调整 prompt 或题型规则库。
