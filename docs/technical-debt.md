...
- 后续遇到新页面或新展示位置时，先判断是否是“通过班级 ID 展示当前正式班级名”；如果是，继续接入命名规则 1。

已确认口径：

- 当前正式班级：凡是通过班级 ID 关联并调取当前班级数据的展示位置，都应接入命名规则 1。
- 页面展示层：默认显示 `年级·班级`，例如 `四年级·1班`。
- 权限判断层：可打开“入学年份”，显示 `入学年份·年级·班级`，例如 `2025级·四年级·1班`。
- 文案统一：该开关和说明统一叫“入学年份”，不叫“入学年级”。
- 历史文本：历史快照、手动填写班级名、旧备注、别名匹配、小程序/家长端兼容字段不自动改名，归入命名规则 2 或历史记录范围，暂不触碰。
- 数据联动：学管中心改了当前班级信息后，所有通过班级 ID 展示的当前班级名应自动跟随变化。
