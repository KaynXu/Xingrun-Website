...
- 当前正式课程名统一走命名规则总入口，再按班型分发到命名规则 1 或命名规则 2。
- 命名规则 1 是班课命名规则，用于多人班课，有班号。
- 命名规则 2 是小课命名规则，用于 `1v1`、`1v2`、`1v3`，没有班号，用学生标识。
- 班课默认展示：`学科·年级·班号班`，例如：`数学·四年级·1班`。
- 班课打开“入学年份”：`学科·学段入学年份级·年级·班号班`，例如：`数学·小2025级·四年级·1班`。
- 小课默认展示：`学科·班型·年级·学生标识`，例如：`数学·1v2·七年级·张李`。
- 小课打开“入学年份”：`学科·班型·学段入学年份级·年级·学生标识`，例如：`数学·1v2·初2025级·七年级·张李`。
- 衔接班按 `A衔B` 的 `B` 判断学段前缀和入学年份，末尾保留 `·小衔初`、`·初衔高`、`·小衔高` 等衔接标识。
- “入学年份”是文案标准，不再使用“入学年级”作为该开关名称。
- 历史快照、手动填写文本、旧备注、别名匹配、小程序/家长端兼容展示等属于历史文本或兼容文本，暂时不追改。
- 学管中心修改当前班级/小课信息后，所有通过班级 ID 展示的当前正式名称应随之变化；历史文本不追改。

已完成：

- 新增 `frontend/src/domain/classNaming.ts`。
- 新增 `frontend/src/classNaming.test.ts`。
- 前端命名总入口已拆为班课命名规则和小课命名规则。
- 后端命名入口已拆为 `build_group_class_name` 和 `build_small_class_name`。
...
- 以后只要遇到“通过班级 ID 展示当前正式课程名”的位置，先判断班型，再接入命名规则 1 或命名规则 2。
- 历史快照、手动填写文本、旧备注、别名匹配、小程序/家长端兼容字段属于历史文本或兼容文本，暂时不碰。
