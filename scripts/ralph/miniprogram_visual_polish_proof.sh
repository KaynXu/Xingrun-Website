...
check('parent-home: primary and secondary actions are grouped by child card', () => {
  ...
  expectIncludes(pages.home.wxml, '我的孩子');
  ...
  expectIncludes(pages.home.wxml, 'class="binding-name-row"');
  expectIncludes(pages.home.wxml, "{{item.className || '未命名班级'}} · {{item.teacherName || '任课老师待补充'}}");
  expectNotIncludes(pages.home.wxml, '上传后可在错题本查看整理进度。');
  expectNotIncludes(pages.home.wxml, 'bindtap="goUpload"');
  expectNotIncludes(pages.home.wxml, 'class="ghost-btn mini-btn current-btn" disabled');
  ...
  expectOrderWithinBlock(pages.home.wxml, 'class="binding-actions"', '</view>', '设为上传', '错题本');
  ...
  expectRule(pages.home.wxss, '.binding-name-row', 'display: flex;');
  ...
  expectRule(pages.home.wxss, '.binding-actions', 'flex: 0 0 auto;');
  expectRule(pages.home.wxss, '.binding-actions', 'align-items: flex-end;');
  expectRule(pages.home.wxss, '.mini-btn', 'min-width: 160rpx;');
...
check('parent-bind: lookup, existing binding, and bind confirmation hierarchy', () => {
  ...
  expectIncludes(pages.bind.wxml, '<button class="ghost-btn" size="mini" bindtap="goHome">去我的</button>');
