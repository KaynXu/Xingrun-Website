...
check('parent-home: primary and secondary actions are grouped by child card', () => {
  ...
  expectOrderWithinBlock(pages.home.wxml, 'class="binding-actions"', '</view>', '设置当前上传孩子', '查看错题本');
  ...
  expectRule(pages.home.wxss, '.binding-card', 'flex-direction: column;');
  expectRule(pages.home.wxss, '.binding-card', 'align-items: stretch;');
