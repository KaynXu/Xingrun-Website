...
- 家长上传页语音转录已切到网站后端本地 `faster-whisper`，当前固定 `base + cpu + int8 + language='zh'`，不再单独要求 OpenAI Whisper key；但语音转成文字后，错因归类仍走现有聊天类 AI provider，所以系统仍需要至少一个可用 provider key。
...
## 本轮完成
- 网站后端录音转文字实现从 OpenAI Whisper 改成了本地 `faster-whisper`，并新增后端单测锁定 `cpu/int8/中文固定` 行为；`requirements.txt` 已补入 `faster-whisper` 依赖。
...
## 剩余问题
- 还没有在真实 4 核 + 4GB 服务器上拿一段中文录音跑过本地 `faster-whisper`，首个请求模型下载、后续 CPU 时延和峰值内存都还需要人工 smoke。
...
## 下一步
- 在目标服务器上先安装新依赖并用一段真实中文短录音走一遍家长上传转录，确认模型首次下载、常驻内存和单次转录时延都能接受；如果 `base` 效果不够，再单独评估是否升到 `small`。
...
## 风险
- 本地 `faster-whisper` 现在固定走 `base + cpu + int8`；这对中文短录音通常够用，但在口音重、环境噪声大或服务器并发高时，识别质量和延迟可能不如云端 Whisper。
