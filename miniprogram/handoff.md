...
## 当前状态
- 2026-05-05 已修复家长绑定页“班级邀请码”输入框文字被裁切/看不见的问题：`pages/parent-bind/index.wxss` 的 `.field-input` 现在显式设置 `width: 100%`、`min-height/line-height: 88rpx` 和水平 padding，复用小程序输入框稳定布局思路；`parent-only-scope.test.js` 已锁定该输入框的宽度、高度、行高和 padding。未改绑定 JS/API 行为。
