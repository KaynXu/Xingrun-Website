...
## 当前状态
- 2026-05-06 已确认家长小程序改为两个底部 Tab 分区：`我的` 用于设置当前上传孩子和新增绑定孩子，`拍照上传` 用于给当前孩子拍照/选图上传错题；设计稿已写入根目录 `docs/superpowers/specs/2026-05-06-miniprogram-parent-two-tab-sections-design.md`。实现尚未开始，范围只应触达小程序前端入口、`parent-home` 和 `parent-upload` 的孩子选择行为，不改上传 API、bridge 或网站后端。
