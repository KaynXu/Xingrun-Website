...
## 当前状态
- 2026-05-03 当前活跃“小程序家长上传 2.0 稳定性”Ralph PRD 在项目根目录 `scripts/ralph/prd.json`；`MP-UPLOAD-001` 到 `MP-UPLOAD-009` 已完成，下一条 story 是 `MP-UPLOAD-010 Refresh parent wrongbook and PDF state after upload`。
- 2026-05-03 `MP-UPLOAD-009` 已完成：网站 worker/API 覆盖识别成功但学生错题库 PDF 重建失败时仍保留原始题图和老师可见错题记录；失败任务状态 payload 会同时给小程序 `parent_error_message` 简短文案和 `maintainer_error_detail` 原始细节，小程序上传摘要优先使用家长文案。
...
- 下一条 Ralph story 是 `MP-UPLOAD-010 Refresh parent wrongbook and PDF state after upload`，重点收上传后的家长错题本刷新和 PDF 未就绪/失败状态。
