...
### Task 1 Spec Compliance Review For `abce9a5` (2026-04-09)
- 结论：FAIL
- 发现：
  - 提交 `abce9a5` 除了 `lesson_manager.py` 和 `tests/test_review_plan_async_store.py` 之外，还修改了 `handoff.md`，不符合 Task 1 的限定文件范围
- 其余对照：
  - commit message 符合 `feat: persist async review generation state`
  - `lesson_manager.py` 已新增 lessons 异步状态字段、迁移补列、`monthly_plan_jobs` 表与所需 helper
  - `tests/test_review_plan_async_store.py` 已覆盖 pending lesson、mark ready、mark failed、monthly pending/ready
- 下一步方向：
  - 若要满足 Task 1 spec，需要移除该提交中的 `handoff.md` 变更后重新提交
