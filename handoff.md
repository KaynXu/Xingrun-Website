...
- 2026-05-04 已修复小程序家长上传语音错因的同步转写超时阻塞：当前小程序 `transcribeParentReason()` 兼容函数改为立即返回空转写，不再发 `/wechat/parent/reason-transcriptions`；bridge 旧同步转写路由也改为校验 `audioUrl` 后立即返回 `{transcript_text: ""}`，避免现场旧包卡在语音转写导致题图任务无法提交。新版上传链路仍是先上传语音文件 URL 和题图任务，后台 `xingrun-rq-worker` 负责语音转写、错因分析、题图识别和 PDF 刷新。latest proof `/tmp/xingrun_async_voice_upload_proof.sh` 已通过：家长小程序测试 71 条、bridge 测试 20 条、bridge build、网站微信上传/错因 flow 56 条、Python compile、`git diff --check`。
