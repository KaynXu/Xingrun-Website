...
### 下一步
- 最邻近的前端收口是把 `smart-wrong-questions.test.ts` 里剩余 2 条“最终错误类型仍是 input”旧断言对齐当前控件形态；这块和本轮 notebook 版式修改无关，尚未一起处理。
...
### 风险
- 当前 `smart-wrong-questions.test.ts` 还剩 2 条旧断言把“最终错误类型”当作 `input`，但工作区里对应控件已经不是这个形态；后续继续做错因下拉时需要先统一测试口径。
