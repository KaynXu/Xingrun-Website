...
### 当前状态
- 2026-04-24 已按标准 release 流程把带错题 PDF 环境净化修复的 `origin/develop(fa7c317)` 合到 `master(31747f5)` 并部署到生产机 `49.234.185.86`。生产机已完成 `git pull --ff-only origin master`、前端 build、`pm2 restart xingrun`，根路由健康检查返回 `HTTP/1.1 302 FOUND`。这次发布同时带上了上一条 `develop` 上的小程序家长端错题最终提交超时修复。发布后已用当前 `xingrun` 的 PM2 进程环境做复现 proof：把 `/proc/<pm2_pid>/environ` 整包灌回 Python 后再次直接调用 `pdf_engine.generate_wrong_question_practice_sheet_pdf(...)`，当前已能在生产机成功生成 `/tmp/xr-pm2-env-proof-sheet21.pdf`，说明之前会把 Chromium 拉崩的 PM2 环境污染链路已经被线上代码截断。
