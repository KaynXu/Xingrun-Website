...
### 当前状态
- 2026-04-21 已按标准 release 流程把错题练习 PDF 崩溃兜底修复发到生产机 `49.234.185.86`：本地 `develop(a52b6bb)` 已合入 `master(cc94d30)` 并推送，生产机已 `git pull --ff-only origin master`、前端 build、`pm2 restart xingrun`，根路由健康检查返回 `HTTP/1.1 302 FOUND`。根因是生产机 snap Chromium 在练习单浏览器 PDF 渲染时仍会偶发 `signal 6` 直接崩溃，而练习单路径没有 ReportLab 回退；当前 `pdf_engine.generate_wrong_question_practice_sheet_pdf()` 已改为浏览器优先、失败后回退 ReportLab，二者都失败时保留浏览器退出码和回退错误。发布前/合并后临时脚本 proof 已跑通后端 `tests.test_wrong_question_library_pdf` + `tests.test_wrong_question_practice_async_api`、前端 `npm test` 183 条、`npm run build`；线上截图里的失败练习单 `sheet-13` 与 `sheet-14` 已重新生成成功，PDF 分别位于 `data/pdfs/wrong_question_practice_sheets/sheet-13.pdf` 和 `sheet-14.pdf`。
