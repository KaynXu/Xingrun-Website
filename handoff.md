## 智能错题双语义风险已再次核对（2026-04-10）

### 已完成
- 已核对当前 handoff 顶部这组“潜在未统一风险”仍然成立，本轮没有改动前后端实现，只补充交接结论，避免下一轮误判为“已经统一语义”。
- 已再次明确当前边界：`wechat_mp` 继续走 `wechat_mastery`，`downstream` 继续走 `downstream_review`；在真正统一契约前，不要在共享前端类型上继续“顺手收口字段”。
- 已再次点名仍需警惕的误写入口：`wrong_question_submissions` 兼容列 `teacher_comment` / `status` 还在，`smart_wrong_questions.py` 下游代理链也还在，所以运行时仍是双来源模型。

### proof
- 临时脚本已校验本文件同时保留“本轮已核对”记录和既有“潜在未统一风险”条目，避免这轮只加结论不落风险正文。

### 剩余问题
- 风险本身没有在这轮被消除；当前只是把交接边界再次钉牢，防止后续维护时把微信错题旧字段重新写回主流程。

### 下一步方向
- 如果下一轮要继续推进，最安全的方式是先决定是否要在后端契约层统一 `wechat_mastery` 与 `downstream_review`，再考虑是否调整共享前端类型；不要只在前端单边改语义。
...
- 完整输出：
  ...
  - `pass 34`
  - `fail 0`
- 临时脚本：系统临时脚本执行 `cd frontend && npx tsx --test src/workspace-dashboard.test.tsx`
- 完整输出：
  - `tests 8`
  - `pass 8`
  - `fail 0`
...
### 剩余问题
- 首页营销文案、隐私条款和其他非错题工作区页面里仍有少量 `题库沉淀 / 资产 / 调用` 一类产品侧表述，本轮未扩 scope 统一改写。

### 下一步方向
- 如果要继续收口前端语言，可以下一轮专门清一遍登录前首页和产品说明页，把“工程/实现视角”进一步换成“老师/机构视角”。
...
### 下一步方向
- 如需继续收口，可以用真实 `owner/admin/super_owner/member` 各点一遍，确认 `admin/owner` 现在是“全机构范围 + 新错题库入口”，`super_owner` 是“跨机构范围 + 新错题库入口”，而 `member` 仍是“本人负责范围 + 新错题库入口”。
...
### 已完成
- 已将 `member` 角色下“学生卡片 -> 错题本”的交互改为点击学生卡片后打开弹窗，不再在页面下半区直接展开详情。
- 弹窗已改为双栏结构：左侧为紧凑 `错题目录`，右侧复用原有详情与编辑区，避免“一题一卡片”导致一屏只能放几题。
- 左侧目录已按高密度行布局显示 `第 N 题 / 题型 / 状态 / 错因 / 时间`，点目录行即可切换当前题目。
- 弹窗右侧现已补成更接近“错题库文档”的版式：顶部增加 `错题档案` 封面和文档导航，再进入详情内容，不再像单纯把原卡片机械塞进弹窗。
- 右侧仍保留现有 `题目文本 / 老师备注 / 处理状态` 等编辑入口，但本轮没有改这些字段的业务含义，只调整承载它们的展示方式。
- 已刻意避免触碰你提到的另一位 AI 正在改的业务逻辑字段，包括 `家长备注 / 重复错题 / 老师处理结果 / 错因归类` 语义，防止并行修改互相覆盖。

### proof
- 验证命令：`cd frontend && npx tsx --test src/smart-wrong-questions.test.ts`
- 完整输出结论：
  - `tests 32`
  - `pass 32`
  - `fail 0`
...
### 剩余问题
- 小程序端仓库不在当前 workspace，本轮未同步修改 mini program 项目。
- 你提到的错因、备注、掌握勾选等字段语义调整应由另一条并行链路继续推进；当前页面只是把这些现有编辑位搬进了 modal。

### 下一步方向
- 等另一位 AI 把错因/备注/掌握逻辑改完后，再做一次字段收口，把弹窗右侧详情里的旧文案与旧字段展示一起替换成新的业务语义；这次右栏版式已经准备好，可以直接承接那批字段替换。
...
### 已完成
- 已继续调整 `frontend/src/App.tsx` 的班级管理 UI：
  - 删除班级卡片摘要里的“点击后弹窗编辑”
  - 将编辑态里的“删除当前班级 / 保存班级”从弹窗底部独立操作栏移入“负责老师”卡片底部
  - 两个按钮改为同级同宽显示，放在负责老师选择区下面
- 已补充 `frontend/src/account-card.test.tsx` 定向源码测试，锁定：
  - 列表卡片里不再出现“点击后弹窗编辑”
  - 删除 / 保存按钮位于负责老师卡片内部，不再位于底部独立操作栏

### proof
- 临时脚本：`/tmp/proof_class_modal_followup_verify_20260409.sh`
- 完整执行结果：
  - `npx tsx --test src/account-card.test.tsx`
    - `tests 44`
    - `pass 44`
...
### 剩余问题
- 当前“编辑学生”卡片仍只支持新增和删除，不支持直接重命名。
- 前端 build 的既有 chunk size warning 仍在，本轮未处理。
...
### 已完成
- 已调整 `frontend/src/App.tsx` 的班级管理编辑弹窗：
  - 删除弹窗标题下“在弹窗里维护班级基础信息、负责老师和家长绑定邀请码。”说明文案
  - 保留第一行“家长绑定邀请码 + 基础信息”卡片布局
  - 将“负责老师”收成与上方卡片同级尺寸，并在其旁边新增“编辑学生”卡片
- 已接入班级学生编辑所需的前端状态和交互：
  - 打开编辑弹窗时自动加载当前班级学生列表
  - 支持在弹窗内新增学生
  - 支持在弹窗内删除学生
- 已补充定向前端源码测试 `frontend/src/account-card.test.tsx`，锁定：
  - 弹窗说明文案已移除
  - “负责老师 + 编辑学生”并排卡片存在
  - 学生编辑卡片包含输入框和删除学生操作
- 已在 `frontend/src/classFeedbackGeneration.ts` 补充 `deleteClassStudent()` helper，复用现有后端班级学生接口

### proof
- 临时脚本：`/tmp/proof_class_modal_verify_20260409.sh`
- 完整执行结果：
  - `npx tsx --test src/account-card.test.tsx`
    - `tests 42`
    - `pass 42`
...
### 剩余问题
- 当前“编辑学生”卡片支持新增和删除，但还不支持直接重命名学生；本轮按最小改动未扩展。
- `vite build` 仍有既有大 chunk warning，本轮未处理，也不影响本轮功能通过。

### 下一步方向
- 如果你下一轮想继续收口班级管理，可以考虑：
  - 给“编辑学生”卡片补充学生重命名
  - 新增学生数统计或更紧凑的学生列表样式
