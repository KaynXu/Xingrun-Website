...
### 当前状态
- 2026-04-20 网站端智能错题这轮继续收口老师端错题练习展示，但当前还只在本地 `develop` 完成、尚未部署：错题练习 PDF 里的非几何题题目区已经去掉 `公式预览 / 预览正常` 等学生无感标签，保留题目卡片本体；两段 AI 挖空提示也已合并成单个大书写区，并在渲染时自动剥离历史数据里的 `小标题：...` 前缀、加宽挖空留白。错题练习记录列表标题不再直接显示数据库 `id`，而是按当前列表顺序显示连续 `练习单 #N`，删除历史练习后不会再出现号码被旧记录占坑；学生错题本和练习记录里的题数/份数文案也已统一改成无空格紧凑写法，避免在卡片里被硬换行成 `6 / 题` 这种断行。
