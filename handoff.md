...
- 当前只改了仓库默认和示例配置，还没有直接登录生产机改 `.env.runtime` 或重启 PM2；生产机如果仍显式设置 `XR_PROVIDER=n1n`，运行时会继续按环境变量优先级使用 n1n，需发版/运维时把生产 `.env.runtime` 改为 `XR_PROVIDER=deepseek` 并配置 `DEEPSEEK_API_KEY`。
