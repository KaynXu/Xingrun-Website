...
### 当前状态
- 2026-04-20 已按标准 release 流程把本地 `develop(c8b13b6)` 合到 `master(5ce8769)` 并部署到生产机 `49.234.185.86`：当前已确认 `local master == origin/master == production HEAD == 5ce8769`，同时 `local develop == origin/develop == c8b13b6`；生产机 `pm2` 服务 `xingrun` 在线，`curl http://127.0.0.1:5001/` 返回 `HTTP/1.1 302 FOUND`。这次发布带上了最近一批 AIPPT 调整，包括叶子提问文案收口，以及“所谓天赋怪就是浅层加工的记忆点停留时间长”这张卡片的新口播文案。
...
- 2026-04-20 错题练习生成链路当前已按业务要求继续收口：几何题在 PDF 里保留原题图片，非几何题走真实题目文本；每题下方不再固定成“把错因补完整 / 下次提醒 / 写一写以后怎么做”这套死模板，也不再出现 `AI 提示` 这类标题。现在由 AI 直接根据该题错因生成两个挖空书写区的小标题和引导内容：一个围绕错因与缺失知识点补空，一个围绕接下来怎么补知识点与以后如何避免再错；两个区块内部不再附送额外纯写字线，页尾会改成补满“重做这题”的答题线，方便学生重新演算。浏览器版 PDF 渲染与 ReportLab fallback 也已补上 `\\(...\\) / \\[...\\]` 公式分隔符支持，当前 proof 已覆盖提示词、LaTeX 渲染和异步生成存储链路。
