...
### 当前状态
- 2026-04-25 `develop` 已完成智能错题练习 PDF 下载命名收口：`/api/wrong-question-practice-sheets/<id>/pdf/download` 现在会用 `孩子姓名联系单_YYYY-MM-DD.pdf` 作为附件下载名，日期取下载当天，孩子姓名为空或包含路径非法字符时会做兜底/清理；PDF 缓存文件路径、预览接口和生成流程不变。前端错题练习记录里的下载链接也已移除旧英文 `download` 文件名，交给后端响应头控制最终保存名。定向回归已补到 `tests/test_smart_wrong_questions_api.py` 和 `frontend/src/smart-wrong-questions.test.ts`，并用临时 proof 脚本跑通对应后端/前端用例。
- 2026-04-24 已定位这次“又崩”的直接原因：不是复习计划整体掉线，而是智能错题练习单的浏览器 PDF 子进程在生产 PM2 环境里再次崩溃。新失败记录是 `wrong_question_practice_sheets.sheet_id=21`，生产日志连续 4 次重试都报 `错题练习 PDF 浏览器渲染失败（signal 6）`。同一份练习单在同机同代码下，只要脱离 PM2 进程环境、改用干净 shell 环境直接调用 `pdf_engine.generate_wrong_question_practice_sheet_pdf(...)` 就能成功生成，说明根因不是题目内容、也不是内存/磁盘不足，而是 Node/Chromium 渲染子进程继承了 PM2 注入的一整包环境变量后再次触发崩溃。本地 `develop` 已补代码级修复：`pdf_engine.py` 现在对错题库/错题练习两个浏览器渲染入口都改成只传最小白名单环境（`PATH/HOME/LANG/XDG_RUNTIME_DIR/DBUS_SESSION_BUS_ADDRESS` 及 `XR_PLAYWRIGHT_* / PLAYWRIGHT_*`），避免把 `NODE_CHANNEL_FD / PM2_HOME / axm_* / status / env` 这类 PM2 元数据继续传给 Chromium；对应回归已补到 `tests/test_wrong_question_library_pdf.py`，并与 `tests/test_wrong_question_practice_async_api.py` 一起通过。线上当前失败的 `sheet_id=21` 也已手工走“重新生成 AI 材料 -> 直接渲染 PDF -> 回写数据库”补救链路恢复为 `ready`，PDF 已写回 `data/pdfs/wrong_question_practice_sheets/sheet-21.pdf`。这次代码修复还没有按 release 流程发到生产，所以生产 `master` 继续生成新练习单时仍有再次撞到同类问题的风险。
