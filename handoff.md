...
### 当前状态
- 2026-04-25 `develop` 已完成智能错题练习 PDF 下载命名收口：`/api/wrong-question-practice-sheets/<id>/pdf/download` 现在会用 `孩子姓名_智能错题_YYYY-MM-DD.pdf` 作为附件下载名，日期取下载当天，孩子姓名为空或包含路径非法字符时会做兜底/清理；PDF 缓存文件路径、预览接口和生成流程不变。定向回归已补到 `tests/test_smart_wrong_questions_api.py`，并用临时 proof 脚本跑通该下载接口用例。
