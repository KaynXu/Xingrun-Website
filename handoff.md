...
### 当前状态
- 2026-05-04 已按用户“发布”确认完成旧小程序语音上传兼容热修 release：`origin/develop(75e355e)` 已合入 `master(7d75fa2)` 并推送 `origin/master`，生产机 `49.234.185.86:/home/ubuntu/Xingrun-Website` 已 fast-forward 到 `7d75fa2a2`。生产已执行数据库初始化、`frontend` build、`miniprogram/backend` build，并重启 `xingrun / xingrun-bridge / xingrun-rq-worker`；三项 PM2 服务均 online，后端根路由返回 `HTTP/1.1 302 FOUND`，bridge `/healthz` 返回 `HTTP/1.1 200 OK`。线上旧接口回归通过：`/wechat/parent/reason-transcriptions` 返回非空 `transcript_text=语音说明已上传，老师端会继续处理。`，`/wechat/parent/reason-classifications` 立即回显 `display_text` 且不再依赖网站 AI。
- 2026-05-04 已按用户“发布”确认完成 release：本地 `develop(840c3b4)` 已合入 `master(5cb183c)` 并推送 `origin/master`，生产机 `49.234.185.86:/home/ubuntu/Xingrun-Website` 已 fast-forward 到 `5cb183c88`。生产已执行数据库初始化、`frontend` build、`miniprogram/backend` build，并重启 `xingrun / xingrun-bridge / xingrun-rq-worker`；三项 PM2 服务均 online，后端根路由返回 `HTTP/1.1 302 FOUND`，bridge `/healthz` 返回 `HTTP/1.1 200 OK`。release proof `/tmp/xingrun_release_production_proof_20260504.sh` 已通过；发布前本地 `develop` 与合并后的 `master` 均已跑通 `/tmp/xingrun_async_voice_upload_proof.sh`，`master` 额外跑通前端 227 条测试和 production build。
