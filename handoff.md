...
- 2026-05-02 已修正 Codex Ralph runner 的模型默认值：本机 `~/.codex/config.toml` 默认 `model="gpt-5.5"`，但当前 `codex-cli 0.118.0` 会被服务端拒绝并提示需要升级 CLI；`scripts/ralph/run_codex_ralph.sh` 现在显式传 `--model "${CODEX_RALPH_MODEL:-gpt-5.4}"`，默认避开该错误，升级 CLI 后仍可用 `CODEX_RALPH_MODEL=gpt-5.5` 覆盖。
