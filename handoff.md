...
### 当前状态
- 2026-04-27 已继续收口智能错题 LaTeX 防线：`frontend/src/wrongQuestionLatex.js` 现在对 KaTeX 无法渲染的坏公式和未闭合 `$...` 不再生成用户可见红色错误块，而是自动降级成普通可读文本；老师端提示文案改为“无法渲染的公式会自动按可读文本显示”。新增回归覆盖非法 `\frac` 与未闭合公式两类场景，latest proof 已通过前端 190 条测试、production build 和 `git diff --check`。注意：这不能保证任意坏 LaTeX 都变成漂亮公式，但能防止预览/导出继续显示 KaTeX parse error 红框。
